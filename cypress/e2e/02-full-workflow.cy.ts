...
describe("Full Inventory Workflow", () => {
  ...
  it("05 - adds items and expenses to supply route", () => {
    ...
    cy.task("dbQuery", `SELECT id FROM supply_routes WHERE name = 'Test Route 47' LIMIT 1`).then(
      (rows: any) => {
        const routeId = rows[0].id
        cy.task("dbQuery", `SELECT id FROM suppliers WHERE name = 'China Textiles Ltd' LIMIT 1`).then(
          (supplierRows: any) => {
            ...
            cy.contains("Container shipping", { timeout: 5000 }).should("be.visible")
