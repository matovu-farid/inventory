...
describe.skip('Mobile POS happy path', () => {
  ...
  before(() => {
    ...
    cy.task(
      'dbQuery',
      `INSERT INTO suppliers (name, type, country) VALUES ('S', 'international', 'CN') ON CONFLICT DO NOTHING`,
    )
    cy.task(
      'dbQuery',
      `INSERT INTO shops (name, location) VALUES ('POS Shop', 'Kampala') ON CONFLICT DO NOTHING`,
    )
    cy.task(
      'dbQuery',
    ...
    cy.request({
      ...
      const cookieStr = Array.isArray(cookie)
        ? cookie.join('; ')
        : (cookie ?? '')
      ...
      cy.request({
        ...
        cy.task(
          'dbQuery',
          `UPDATE "user" SET email_verified = TRUE WHERE email = '${salesEmail}'`,
        )
        cy.task(
          'dbQuery',
  ...
  it('creates a sale via the mobile POS flow', () => {
    ...
    cy.contains('button', /Confirm sale/i, { timeout: 5000 }).click({
      force: true,
    })
