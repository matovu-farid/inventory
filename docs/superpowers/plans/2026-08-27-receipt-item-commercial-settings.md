...
`12000`, and the second line is `15000`. Add a duplicate-row test that expects
conflicting defaults to be rejected.
...
line’s effective minimum price instead. Track resolved item defaults in the
transaction and throw:
...
- Create: `drizzle/0022_receipt_item_defaults.sql`
...
threshold reads back `0`, and that `NULL` is no longer accepted.

- [ ] **Step 2: Make the schema non-null with a zero default**

Change the Drizzle field to:
...
The migration must backfill existing nulls before adding the constraint:
...
### Task 4: Remove live-item pricing fallbacks from stock movement and sales

**Files:**
- Modify: `src/server/functions/shop/sales.ts`
- Modify: `src/server/functions/store/transfers.ts`
- Modify: `src/server/functions/store/fifo.ts`
- Modify: `src/server/functions/shop/fifo.ts`
- Modify: `src/routes/items/$articleNumber.tsx`
- Modify: `src/routes/store/index.tsx`
- Test: the existing store transfer, shop sales, FIFO, and item-detail tests

- [ ] **Step 1: Write failing snapshot-invariance tests**

Create stock with a stored minimum sell price of `12000`, update the item
default to `15000`, then assert FIFO/sale/transfer planning still uses
`12000`. Also assert a stored `0` remains `0` and does not fall back to `15000`.
...
`lowStockThreshold`. Confirm that no stock/sale path can substitute a live item
default for a non-null stock snapshot, and that every receipt entry path sets
threshold `0` when blank.
