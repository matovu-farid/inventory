...
- [ ] **Step 2: Test Supplier Routes.** Open a local route entry and click through Items. Search an existing item by art number and design, select it, type a free-text row, enter comma-separated colours and sizes, open the picker and drag its pointer without dismissal, add/delete a row, click the trailing row, fill a value beyond the current last row, undo/redo, trigger a search failure if the local mock/server permits it and verify typed text remains, and move to Review. Verify item name, design, art number, colours, sizes, quantities, amounts, and commercial values are shown.
