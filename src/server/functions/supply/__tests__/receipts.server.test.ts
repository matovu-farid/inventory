...
vi.mock('#/server/middleware/auth', () => ({
  requireSession: () => Promise.resolve({ user: { id: TEST_USER_ID, role: 'admin' } }),
}))
vi.mock('#/server/middleware/rbac', () => ({
  requireRole: () => {},
  hasRole: () => true,
  requireSessionAndRole: () => Promise.resolve({ user: { id: TEST_USER_ID, role: 'admin' } }),
}))
...
beforeAll(async () => {
  const [supplier] = await db
    .insert(suppliers)
    .values({ name: `Receipt auto item supplier ${suffix}`, type: 'international' })
    .returning()
...
afterAll(async () => {
  ...
  if (createdItemIds.length > 0) {
    await db.delete(itemArticleNumbers).where(inArray(itemArticleNumbers.itemId, createdItemIds))
...
function draft(design: string, articleNumber: string) {
  return {
...
describe('receipt item materialization', () => {
  it('creates an item and attaches its receipt art number', async () => {
    ...
    const result = await createSupplyRouteReceiptServer(draft(design, articleNumber))
    const created = await db.query.items.findFirst({ where: eq(items.design, design) })
  ...
  it('reuses an item for the same design and adds a second art number', async () => {
    const design = `Receipt jacket ${suffix}`
    const result = await createSupplyRouteReceiptServer(draft(design, `RB-${suffix}`))
    const item = await db.query.items.findFirst({ where: eq(items.design, design) })
    const numbers = item
      ? await db.query.itemArticleNumbers.findMany({ where: eq(itemArticleNumbers.itemId, item.id) })
      : []

    expect(result.lines[0].itemId).toBe(item?.id)
    expect(numbers.map((number) => number.articleNumber)).toEqual(
      expect.arrayContaining([`RA-${suffix}`.toUpperCase(), `RB-${suffix}`.toUpperCase()]),
    )
  ...
  it('rejects an art number already owned by another item', async () => {
    await expect(
      createSupplyRouteReceiptServer(draft(`Different design ${suffix}`, `RA-${suffix}`)),
