...
export function CheckoutSheet({
  ...
  const [paymentMethod, setPaymentMethod] = React.useState<'cash' | 'bank'>(
    'cash',
  )
  const [submitting, setSubmitting] = React.useState(false)
  const [completedSaleId, setCompletedSaleId] = React.useState<string | null>(
    null,
  )
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setCompletedSaleId(
          result.mode === 'online' ? result.saleId : result.localId,
        )
        setStage('success')
      } else {
            ...
            {state.items.map((i) => (
              ...
              >
                ...
                <span className="font-mono">
                  {formatUgxTotal(Number(i.unitPriceUgx) * i.qty)}
                </span>
              </div>
            ))}
