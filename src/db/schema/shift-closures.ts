...
export const shiftClosures = pgTable(
  'shift_closures',
  {
    ...
    openingCashUgx: numeric('opening_cash_ugx', { precision: 15, scale: 2 })
      .notNull()
      .default('0'),
    declaredCashUgx: numeric('declared_cash_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
    expectedCashUgx: numeric('expected_cash_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
    varianceUgx: numeric('variance_ugx', { precision: 15, scale: 2 }).notNull(),
    grossSalesUgx: numeric('gross_sales_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
    cashSalesUgx: numeric('cash_sales_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
    bankSalesUgx: numeric('bank_sales_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
    creditSalesUgx: numeric('credit_sales_ugx', {
      precision: 15,
      scale: 2,
    }).notNull(),
