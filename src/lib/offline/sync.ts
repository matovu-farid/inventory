...
async function drainQueue(): Promise<void> {
  ...
  try {
    ...
    const pending = all.filter((s) => s.status === "queued" || s.status === "syncing")
...
export function useSyncEngine(): SyncCounts & { refresh: () => Promise<void> } {
  ...
  const [counts, setCounts] = React.useState<SyncCounts>({ queued: 0, failed: 0 })

  async function refreshCounts() {
    try {
      const all = await getQueuedSales()
      setCounts({
        queued: all.filter((s) => s.status === "queued" || s.status === "syncing").length,
