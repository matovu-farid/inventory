...
const ITEM_A = { itemId: 'variant-a' }
const ITEM_B = { itemId: 'variant-b' }
const SHOP_1 = 'shop-1'
const SHOP_2 = 'shop-2'
...
describe('resolveShopRule', () => {
  it('returns global shop default when no override matches', () => {
    ...
    const rule = resolveShopRule(SHOP_1, ITEM_A, maps, defaults)
  ...
  it('prefers shop+variant override over variant-only override', () => {
    ...
    const maps = buildOverrideMaps(overrides)
    expect(resolveShopRule(SHOP_1, ITEM_A, maps, defaults)).toEqual({
      mode: 'percent',
      value: 25,
    })
    // SHOP_2 still sees the variant-only override
    expect(resolveShopRule(SHOP_2, ITEM_A, maps, defaults)).toEqual({
      mode: 'units',
      value: 10,
    })
  })
  ...
  it('does not bleed across variants', () => {
    ...
    const maps = buildOverrideMaps(overrides)
    expect(resolveShopRule(SHOP_1, ITEM_B, maps, defaults)).toEqual(
      defaults.shop,
    )
  })
})
...
describe('resolveStoreRule', () => {
  it('returns global store default when no override matches', () => {
    const maps = buildOverrideMaps([])
    expect(resolveStoreRule(ITEM_A, maps, defaults)).toEqual(defaults.store)
  })
  ...
  it('prefers variant override over global', () => {
    ...
    const maps = buildOverrideMaps(overrides)
    expect(resolveStoreRule(ITEM_A, maps, defaults)).toEqual({
      mode: 'units',
      value: 50,
    })
  })
  ...
  it('does not bleed across variants', () => {
    ...
    const maps = buildOverrideMaps(overrides)
    expect(resolveStoreRule(ITEM_B, maps, defaults)).toEqual(defaults.store)
  })
})
