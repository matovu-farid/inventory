...
} from '#/server/functions/admin/item-categories.server'
...
afterAll(async () => {
  if (createdIds.length > 0) {
    await db
      ...
      .where(inArray(itemCategories.id, createdIds))
  }
  // Cleanup any orphans created by individual tests.
  await db
    .delete(itemCategories)
    .where(inArray(itemCategories.name, [NAME_A, NAME_B, NAME_RENAMED]))
})
