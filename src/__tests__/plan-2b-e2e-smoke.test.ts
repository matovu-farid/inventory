...
describe("Plan 2b — end-to-end smoke", () => {
  it("unresolved chain: sell → customer return → dispatch → store receipt", async () => {
    const itemId = await seedItem({
      articleNumber: "E2E-1", name: "Polo", minimumSellPriceUgx: "100.00",
    })
    const shopId = await seedShop()
    const storeId = await seedStore()
    const supplyLine = await seedSupplyRouteLine({ itemId, colorId: null, size: null })
    const { stockId } = await seedShopStockLot({
      shopId, itemId, variantId: null,
      supplyRouteLineId: supplyLine, quantity: 10, costPerUnitUgx: "60.00",
    })
    ...
    const saleLine = (await db.query.shopSaleLines.findMany({
    ...
    expect((await db.query.shopStock.findFirst({
      where: eq(shopStock.id, stockId),
    }))?.quantityOnHand).toBe(6)
    ...
    const retLine = (await db.query.shopReturnLines.findMany({
    ...
    const storeRetLine = (await db.query.storeReturnLines.findMany({
    ...
    await callServerFn(() =>
      receiveStoreReturn({
        data: {
          ...
          itemReceipts: [{ storeReturnItemId: storeRetLine.id, quantityReceived: 5 }],
        },
      }),
    )
  ...
  it("variant chain: same flow with fully-resolved variants survives the round trip", async () => {
    const itemId = await seedItem({
      articleNumber: "E2E-2", name: "Tee", minimumSellPriceUgx: "100.00",
    })
    ...
    const { stockId, variantId } = await seedShopStockLot({
      ...
      supplyRouteLineId: supplyLine, quantity: 10, costPerUnitUgx: "70.00",
    })
    ...
    await callServerFn(() =>
      recordCustomerReturn({
        data: {
          ...
          reason: "doesn't fit",
          refundMethod: "cash",
          items: [{ itemId, variantId, quantity: 1, unitRefundPriceUgx: "150.00" }],
        },
      }),
    )
    expect((await db.query.shopStock.findFirst({
    ...
    await callServerFn(() =>
      dispatchStoreReturn({
        data: {
          ...
          items: [{
            itemId, variantId,
            quantityDispatched: 4, unitTransferPriceUgx: "150.00",
          }],
        },
      }),
    )
    ...
    await callServerFn(() =>
      receiveStoreReturn({
        data: {
          storeReturnId: storeRet.id,
          itemReceipts: [{ storeReturnItemId: storeRetLine.id, quantityReceived: 4 }],
        },
      }),
    )
    const storeRows = await db.query.storeStock.findMany({
